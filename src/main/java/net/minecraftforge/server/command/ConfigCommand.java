        ...
        private static int showFile(final CommandContext<ServerCommandSource> context) {
            ...
            if (configFileName != null) {
                ...
                context.getSource().sendFeedback(new TranslatableText("commands.config.getwithtype",
                        modId, type,
                        new LiteralText(f.getName()).formatted(Formatting.field_1073).
                                styled((style) -> style.setClickEvent(new ClickEvent(ClickEvent.Action.field_11746, f.getAbsolutePath())))
                ), true);
            } else {
