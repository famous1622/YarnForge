	...
	private static class EntityListCommand {
		...
		static ArgumentBuilder<ServerCommandSource, ?> register() {
			return CommandManager.literal("list")
					...
					.then(CommandManager.argument("filter", StringArgumentType.string())
							.suggests((ctx, builder) -> CommandSource.suggestMatching(ForgeRegistries.ENTITIES.getKeys().stream().map(id -> id.toString()), builder))
		...
		private static int execute(ServerCommandSource sender, String filter, DimensionType dim) throws CommandSyntaxException {
			...
			if (names.size() == 1) {
				...
				sender.sendFeedback(new TranslatableText("commands.forge.entity.list.single.header", name, info.getLeft()), true);
				...
				for (Map.Entry<ChunkPos, Integer> e : toSort) {
					if (limit-- == 0) break;
					sender.sendFeedback(new LiteralText("  " + e.getValue() + ": " + e.getKey().x + ", " + e.getKey().z), true);
				}
				return toSort.size();
			} else {
				...
				sender.sendFeedback(new TranslatableText("commands.forge.entity.list.multiple.header", count), true);
				info.forEach(e -> sender.sendFeedback(new LiteralText("  " + e.getValue() + ": " + e.getKey()), true));
				return info.size();
			}
		}
	}
