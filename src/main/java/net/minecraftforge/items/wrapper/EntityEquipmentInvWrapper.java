...
public abstract class EntityEquipmentInvWrapper implements IItemHandlerModifiable {
	...
	public ItemStack insertItem(final int slot, @Nonnull final ItemStack stack, final boolean simulate) {
		...
		if (!simulate) {
			if (existing.isEmpty()) {
				entity.setEquippedStack(equipmentSlot, reachedLimit ? ItemHandlerHelper.copyStackWithSize(stack, limit) : stack);
			} else {
	...
	public ItemStack extractItem(final int slot, final int amount, final boolean simulate) {
		...
		if (existing.getCount() <= toExtract) {
			if (!simulate) {
				entity.setEquippedStack(equipmentSlot, ItemStack.EMPTY);
			}

			return existing;
		} else {
			if (!simulate) {
				entity.setEquippedStack(equipmentSlot, ItemHandlerHelper.copyStackWithSize(existing, existing.getCount() - toExtract));
			}
	...
	public void setStackInSlot(final int slot, @Nonnull final ItemStack stack) {
		...
		entity.setEquippedStack(equipmentSlot, stack);
	}
