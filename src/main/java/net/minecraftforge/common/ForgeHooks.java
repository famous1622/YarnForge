...
public class ForgeHooks
{
    ...
    static void initTools()
    {
        ...
        blockToolSetter.accept(Blocks.field_10540, ToolType.PICKAXE, 3);
        for (Block block : new Block[]{Blocks.field_10201, Blocks.field_10442, Blocks.field_10013, Blocks.field_10234, Blocks.field_10205, Blocks.field_10571, Blocks.field_10080})
            blockToolSetter.accept(block, ToolType.PICKAXE, 2);
        for (Block block : new Block[]{Blocks.field_10085, Blocks.field_10212, Blocks.field_10441, Blocks.field_10090})
    ...
    public static boolean onPickBlock(HitResult target, PlayerEntity player, World world)
    {
        ...
        if (target.getType() == HitResult.Type.field_1332)
        {
        ...
        else if (target.getType() == HitResult.Type.field_1331)
        {
        ...
        if (isCreative)
        {
            ...
            MinecraftClient.getInstance().interactionManager.clickCreativeStack(player.getStackInHand(Hand.field_5808), 36 + player.inventory.selectedSlot);
            return true;
        }
    ...
    public static Text newChatWithLinks(String string, boolean allowMissingHeader)
    {
        ...
        while (matcher.find())
        {
            ...
            ClickEvent click = new ClickEvent(ClickEvent.Action.field_11749, url);
            link.getStyle().setClickEvent(click);
            link.getStyle().setUnderline(true);
            link.getStyle().setColor(Formatting.field_1078);
            if (ichat == null)
    ...
    public static int onBlockBreakEvent(World world, GameMode gameType, ServerPlayerEntity entityPlayer, BlockPos pos)
    {
        ...
        {
            if (gameType == GameMode.field_9219)
                preCancelEvent = true;
