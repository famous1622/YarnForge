...
public class DimensionManager
{
    ...
    {
        Validate.isTrue(dim != DimensionType.field_13072, "Can not hotload overworld. This must be loaded at all times by main Server.");
        Validate.notNull(server, "Must provide server when creating world");
        Validate.notNull(dim, "Must provide dimension when creating world");

        ServerWorld overworld = getWorld(server, DimensionType.field_13072, false, false);
    ...
    {
        return world.getDimension().getType() != DimensionType.field_13072
