...
public class DimensionManager {
	...
	public static void readRegistry(CompoundTag data) {
		...
		for (int x = 0; x < list.size(); x++) {
			SavedEntry entry = new SavedEntry(list.getCompoundTag(x));
			if (entry.type == null) {
	...
	public static class SavedEntry {
		...
		private SavedEntry(CompoundTag data) {
			...
			this.type = data.containsKey("type", 8) ? new Identifier(data.getString("type")) : null;
			this.data = data.containsKey("data", 7) ? data.getByteArray("data") : null;
			this.skyLight = !data.containsKey("sky_light", 99) || data.getBoolean("sky_light");
		}
	...
	private static class NoopChunkStatusListener implements WorldGenerationProgressListener {
		...
		@Override
		public void setChunkStatus(ChunkPos p_219508_1_, ChunkStatus p_219508_2_) {
		}
