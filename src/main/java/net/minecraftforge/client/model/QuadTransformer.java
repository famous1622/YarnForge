...
public class QuadTransformer
{
    ...
    private static int findPositionOffset(VertexFormat fmt)
    {
        ...
        for (index = 0; index < fmt.getElements().size(); index++)
        {
            ...
            if (el.getType() == VertexFormatElement.Type.field_1633)
            {
                element = el;
                break;
            }
        ...
        if (element.getFormat() != VertexFormatElement.Format.field_1623)
    ...
    private static int findNormalOffset(VertexFormat fmt)
    {
        ...
        {
            VertexFormatElement el = fmt.getElements().get(index);
            if (el.getType() == VertexFormatElement.Type.field_1635)
            {
                element = el;
                break;
            }
        ...
        if (element.getFormat() != VertexFormatElement.Format.field_1621)
    ...
    public BakedQuad processOne(BakedQuad input)
    {
        ...
        return new BakedQuad(outData, input.getColorIndex(), input.getFace(), input.func_187508_a(), input.shouldApplyDiffuseLighting());
    }
    ...
    public List<BakedQuad> processMany(List<BakedQuad> inputs)
    {
        ...
        for(BakedQuad input : inputs)
        {
            ...
            outputs.add(new BakedQuad(outData, input.getColorIndex(), input.getFace(), input.func_187508_a(), input.shouldApplyDiffuseLighting()));
        }
        return outputs;
    }
