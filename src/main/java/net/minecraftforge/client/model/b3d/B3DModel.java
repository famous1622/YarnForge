	...
	public static class Parser {
				...
				if (oldKey != null) {
					if (pos != null) {
						if (oldKey.getPos() != null) {
							logger.error("Duplicate keys: {} and {} (ignored)", oldKey, key);
						} else {
		...
		private Node<?> node() throws IOException {
			...
			if (mesh != null) {
				...
				node = Node.create(name, pos, scale, rot, nodes, new Bone(bone));
			} else {
				node = Node.create(name, pos, scale, rot, nodes, new Pivot());
			}
			if (animData == null) {
	...
	public static class Vertex {
		...
		public Vertex bake(Mesh mesh, Function<Node<?>, Matrix4f> animator) {
			...
			if (mesh.getWeightMap().get(this).isEmpty()) {
				...
				if (Math.abs(totalWeight) > 1e-4) {
					t.mul(1f / totalWeight);
				} else {
					t.setIdentity();
				}
			}
