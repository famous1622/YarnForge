...
public final class DynamicBucketModel implements IModelGeometry<DynamicBucketModel>
{
    ...
    public net.minecraft.client.render.model.BakedModel bake(IModelConfiguration owner, ModelLoader bakery, Function<SpriteIdentifier, Sprite> spriteGetter, ModelBakeSettings modelTransform, ModelItemPropertyOverrideList overrides, Identifier modelLocation)
    {
        ...
        if (flipGas && fluid != Fluids.field_15906 && fluid.getAttributes().isLighterThanAir())
        {
        ...
        Sprite fluidSprite = fluid != Fluids.field_15906 ? spriteGetter.apply(ForgeHooksClient.getBlockMaterial(fluid.getAttributes().getStillTexture())) : null;
        ...
        if (fluidMaskLocation != null && fluidSprite != null)
        {
            ...
            if (templateSprite != null)
            {
                ...
                builder.addAll(ItemTextureQuadConverter.convertTexture(transform, templateSprite, fluidSprite, NORTH_Z_FLUID, Direction.field_11043, tint ? fluid.getAttributes().getColor() : 0xFFFFFFFF, 1));
                builder.addAll(ItemTextureQuadConverter.convertTexture(transform, templateSprite, fluidSprite, SOUTH_Z_FLUID, Direction.field_11035, tint ? fluid.getAttributes().getColor() : 0xFFFFFFFF, 1));
            }
        }
        ...
        if (coverLocation != null && (!coverIsMask || baseLocation != null))
        {
            ...
            if (coverSprite != null)
            {
                if (coverIsMask)
                {
                    ...
                    builder.addAll(ItemTextureQuadConverter.convertTexture(transform, coverSprite, baseSprite, NORTH_Z_COVER, Direction.field_11043, 0xFFFFFFFF, 1));
                    builder.addAll(ItemTextureQuadConverter.convertTexture(transform, coverSprite, baseSprite, SOUTH_Z_COVER, Direction.field_11035, 0xFFFFFFFF, 1));
                }
                else
                {
                    builder.add(ItemTextureQuadConverter.genQuad(transform, 0, 0, 16, 16, NORTH_Z_COVER, coverSprite, Direction.field_11043, 0xFFFFFFFF, 2));
                    builder.add(ItemTextureQuadConverter.genQuad(transform, 0, 0, 16, 16, SOUTH_Z_COVER, coverSprite, Direction.field_11035, 0xFFFFFFFF, 2));
