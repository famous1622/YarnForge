...
public final class FMLWorldPersistenceHook implements WorldPersistenceHooks.WorldPersistenceHook {
	...
	public void readData(WorldSaveHandler handler, LevelProperties info, CompoundTag tag) {
		if (tag.containsKey("LoadingModList")) {
			ListTag modList = tag.getList("LoadingModList", (byte) 10);
			for (int i = 0; i < modList.size(); i++) {
				CompoundTag mod = modList.getCompoundTag(i);
		...
		if (tag.containsKey("ModItemData") || tag.containsKey("ItemData")) // Pre 1.7
		{
			StartupQuery.notify("This save predates 1.7.10, it can no longer be loaded here. Please load in 1.7.10 or 1.8 first");
			StartupQuery.abort();
		} else if (tag.containsKey("Registries")) // 1.8, genericed out the 'registries' list
		{
