...
public class ServerLifecycleHooks {
	...
	public static boolean handleServerAboutToStart(final MinecraftServer server) {
		...
		ResourcePackLoader.loadResourcePacks(currentServer.getDataPackContainerManager(), ServerLifecycleHooks::buildPackFinder);
...
*/
	...
	private static <T extends ResourcePackContainer> void serverPackFinder(Map<ModFile, ? extends ModFileResourcePack> modResourcePacks, BiConsumer<? super ModFileResourcePack, ? super T> packSetter, Map<String, T> packList, ResourcePackContainer.Factory<? extends T> factory) {
		for (Entry<ModFile, ? extends ModFileResourcePack> e : modResourcePacks.entrySet()) {
			IModInfo mod = e.getKey().getModInfos().get(0);
			if (Objects.equals(mod.getModId(), "minecraft")) continue; // skip the minecraft "mod"
			final String name = "mod:" + mod.getModId();
			final T packInfo = ResourcePackContainer.of(name, true, e::getValue, factory, ResourcePackContainer.InsertionPosition.TOP);
			if (packInfo == null) {
