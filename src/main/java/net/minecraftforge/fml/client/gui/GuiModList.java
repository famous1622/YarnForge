...
public class GuiModList extends Screen {
	...
	public void init() {
		...
		this.addButton(this.openModsFolderButton = new ButtonWidget(6, this.height - 24, this.listWidth, 20,
				I18n.translate("fml.menu.mods.openmodsfolder"), b -> SystemUtil.getOperatingSystem().open(FMLPaths.MODSDIR.get().toFile())));
	...
	class InfoPanel extends ScrollPanel {
		...
		private List<Text> resizeContent(List<String> lines) {
			...
			for (String line : lines) {
				...
				if (maxTextLength >= 0) {
					ret.addAll(TextComponentUtil.wrapLines(chat, maxTextLength, GuiModList.this.font, false, true));
				}
			}
			return ret;
		}
