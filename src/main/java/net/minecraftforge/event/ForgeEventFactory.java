...
public class ForgeEventFactory {
	...
	public static boolean canMountEntity(Entity entityMounting, Entity entityBeingMounted, boolean isMounting) {
		...
		if (isCanceled) {
			entityMounting.setPositionAnglesAndUpdate(entityMounting.x, entityMounting.y, entityMounting.z, entityMounting.prevYaw, entityMounting.prevPitch);
			return false;
		} else {
			return true;
		}
	...
	public static boolean fireSleepingTimeCheck(PlayerEntity player, Optional<BlockPos> sleepingLocation) {
		...
		Result canContinueSleep = evt.getResult();
		if (canContinueSleep == Result.DEFAULT) {
			return !player.world.isDaylight();
		} else {
			return canContinueSleep == Result.ALLOW;
		}
	}
	...
	public static LootSupplier loadLootTable(Identifier name, LootSupplier table, LootManager lootTableManager) {
		LootTableLoadEvent event = new LootTableLoadEvent(name, table, lootTableManager);
		if (MinecraftForge.EVENT_BUS.post(event)) {
			return LootSupplier.EMPTY;
		}
