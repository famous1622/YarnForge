...
public class FluidAttributes
{
    ...
    {
        worldIn.playSound(player, pos, SoundEvents.field_15102, SoundCategory.field_15245, 0.5F, 2.6F + (worldIn.random.nextFloat() - worldIn.random.nextFloat()) * 0.8F);

        for (int l = 0; l < 8; ++l)
        {
            worldIn.addImportantParticle(ParticleTypes.field_11237, (double) pos.getX() + Math.random(), (double) pos.getY() + Math.random(), (double) pos.getZ() + Math.random(), 0.0D, 0.0D, 0.0D);
        }
    }
